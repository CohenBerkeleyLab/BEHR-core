function [ behr_flags, flags_meaning ] = behr_quality_flags( behr_amfs, behr_vis_amfs, vcd_flags, xtrack_flags, modis_ocean_flag )
...
flags_meaning{1} = '1: Summary bit: set if any error bit set';
...
behr_flags = set_flags(behr_flags, mod(vcd_flags, 2) ~= 0, 3, false,...
    'VcdQualityFlags: NASA summary flag set');
...
behr_flags = set_flags(behr_flags, xtrack_flags ~= 0, 4, false,...
    'XTrackQualityFlags: NASA flag > 0');
...
behr_flags = set_flags(behr_flags, modis_ocean_flag, 17, true,...
    'Ocean Albedo Flag: surface albedo uses Mobley LUT');

    function flags = set_flags(flags, bool_mask, bit, warning_only, explanation_string)
