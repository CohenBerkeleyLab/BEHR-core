...
for a=1:numel(OMI)
    ...
    if usebehr
        omi = omi_pixel_reject(omi, cloud_prod, cloud_frac_max, row_anomaly); %We will set the area weight to 0 for any elements that should not contribute to the average
    else
        omi = omi_sp_pixel_reject(omi, cloud_prod, cloud_frac_max, row_anomaly);
    end
