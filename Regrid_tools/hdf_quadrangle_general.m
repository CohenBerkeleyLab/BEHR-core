function OMI = hdf_quadrangle_general(Data, maxx, minx, maxy, miny, lCoordLon, lCoordLat, Lon1, Lon2, Lon4, Lat1, Lat2, Lat4)

% hdf_quadrangle_general: Version of hdf_quadrangle_5km_new by Ashley
% Russell from 11/19/2009 that has been generalized for any satellite 
%
...
if numel(varargin)==1
    tmp_fields = varargin{1};
elseif numel(varargin)==2
    reg_fields = varargin{1};
    flag_fields = varargin{2};
else
    tmp_fields = fieldnames(Data);
end

% If two lists of fields were not passed, parse the field names to find any
% containing the string 'flag'.  These will be assumed to be quality flags
% (which a typically a bit array) and thus not averaged.
if numel(varargin)~=2
    reg_fields = {};
    flag_fields = {};
    for a=1:numel(tmp_fields);
        re = regexpi(tmp_fields{a},'flags');
        if isempty(re)
            reg_fields{end+1} = tmp_fields{a};
        else
            flag_fields{end+1} = tmp_fields{a};
        end
    end
end

% Additional fields that must be added. Check that these are not in either
% of the field lists, if they are, print a warning that they will not be
% copied in.  Since the pixel field is intended to keep

if any(strcmp('Count',reg_fields)); warning('Count will not be copied from the Data structure; the OMI field Count will contain other information'); 
end

if any(strcmp('Area',reg_fields)); warning('Area will not be copied from the Data structure; the OMI field Area will contain the pixel''s footprint area'); 
end

if any(strcmp('Areaweight',reg_fields)); warning('Areaweight will not be copied from the Data structure; the OMI field Areweight will contain a weight of 1/Area'); 
end
...
% arrays instead.
for a=1:numel(reg_fields)
    eval(sprintf('%s=zeros(maxx,maxy);',reg_fields{a}));
end
for a=1:numel(flag_fields)
    eval(sprintf('%s=cell(maxx,maxy);',flag_fields{a}));
end
...
TraceGasFieldMat = eval(sprintf('Data.%s',TraceGasField));

Dimensions = size(TraceGasFieldMat);
for x=1:1:Dimensions(1)*Dimensions(2); %JLL 18 Mar 2014: Loop over each NO2 column in Data(d)
    ...
    for a=1:numel(reg_fields)
        eval(sprintf('%s_val = Data.%s(x);',reg_fields{a},reg_fields{a}))
    end
    for a=1:numel(flag_fields)
        eval(sprintf('%s_val = Data.%s(x);',flag_fields{a}, flag_fields{a}));
    end
    ...
    for y_quad=bottom:1:top; %JLL 19 Mar 2014:
        ...
        elseif (maxx>=right) && (right>=1) && (1<=left) && (left<maxx); %JLL 19 Mar 2014: Make sure the left and right bounds are inside the permissible limits
            ...
            for x_quad=left+1:right;
                ...
                % existing average
                if eval(sprintf('%s(x_quad, y_quad)',TraceGasField)) ~= 0 && ~isnan(TraceGasFieldMat(x,y)); 
                    ...
                    for a=1:numel(reg_fields)
                        eval(sprintf('%s(x_quad, y_quad) = nansum([%s(x_quad, y_quad)*(Count(x_quad, y_quad)-1), %s_val])/Count(x_quad,y_quad);', reg_fields{a}, reg_fields{a}, reg_fields{a}));
                    end
                    
                    % Flag fields will append the flag value to a matrix in
                    % a cell corresponding to this grid cell
                    for a=1:numel(flag_fields)
                        eval(sprintf('%s(x_quad, y_quad) = {[%s{x_quad, y_quad}, %s_val]};', flag_fields{a}, flag_fields{a}, flag_fields{a}))
                    end
                
                % If there is no existing field
                elseif ~isnan(TraceGasFieldMat(x,y)) %JLL 19 Mar 2014: I added the logical test here, before this was just an 'else' statement, but it would make sense not to add a value if there was no valid NO2 column.
                    % Count, area, and areaweight require special handling
                    Count(x_quad,y_quad)=Count(x_quad,y_quad)+1;
                    ...
                    % Regular fields will be a running average
                    for a=1:numel(reg_fields)
                        eval(sprintf('%s(x_quad, y_quad) = %s_val;', reg_fields{a}, reg_fields{a}));
                    end
                    
                    % Flag fields will append the flag value to a matrix in
                    % a cell corresponding to this grid cell
                    for a=1:numel(flag_fields)
                        eval(sprintf('%s(x_quad, y_quad) = {%s_val};', flag_fields{a}, flag_fields{a}))
                    end
                end
            end
        end
...
for a=1:numel(reg_fields)
    eval(sprintf('OMI.%s = %s',reg_fields{a}));
end
for a=1:numel(flag_fields)
    eval(sprintf('OMI.%s = %s',flag_fields{a}));
end
