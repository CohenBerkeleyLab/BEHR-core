function OMI = hdf_quadrangle_BEHR_InSitu(Data, OMI, maxx, minx, maxy, miny, lCoordLon, lCoordLat, Lon1, Lon2, Lon4, Lat1, Lat2, Lat4)
...
InSituAMF=fill_val * ones(maxx,maxy);
BEHR_R_ColumnAmountNO2Trop=fill_val * ones(maxx,maxy);
...
for x=1:1:Dimensions(1)*Dimensions(2); %JLL 18 Mar 2014: Loop over each NO2 column in Data(d)
    ...
    InSituAMF_val = Data.InSituAMF(x);
    BEHR_R_ColumnAmountNO2Trop_val = Data.BEHR_R_ColumnAmountNO2Trop(x);
    ...
    for y_quad=bottom:1:top; %JLL 19 Mar 2014:
        ...
        elseif (maxx>=right) && (right>=1) && (1<=left) && (left<maxx); %JLL 19 Mar 2014: Make sure the left and right bounds are inside the permissible limits
            ...
            for x_quad=left+1:right;
                ...
                InSituAMF(x_quad, y_quad) = sum([InSituAMF(x_quad, y_quad)*(Count(x_quad, y_quad)-1), InSituAMF_val])/Count(x_quad,y_quad);
                BEHR_R_ColumnAmountNO2Trop(x_quad, y_quad) = sum([BEHR_R_ColumnAmountNO2Trop(x_quad, y_quad)*(Count(x_quad, y_quad)-1), BEHR_R_ColumnAmountNO2Trop_val])/Count(x_quad,y_quad);
                ...
                % If there is no existing field
                 elseif ~isnan(BEHRColumnAmountNO2Trop_val) %JLL 19 Mar 2014: I added the logical test here, before this was just an 'else' statement, but it would make sense not to add a value if there was no valid NO2 column.
                    ...
                    InSituAMF(x_quad, y_quad) = InSituAMF_val;
                    BEHR_R_ColumnAmountNO2Trop(x_quad, y_quad) = BEHR_R_ColumnAmountNO2Trop_val;
...
OMI.InSituAMF = InSituAMF;
OMI.BEHR_R_ColumnAmountNO2Trop = BEHR_R_ColumnAmountNO2Trop;
OMI.Time = Time;
