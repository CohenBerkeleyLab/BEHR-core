...
p = inputParser;
p.addParameter('behrdir',fullfile(HOME,'Documents', 'MATLAB', 'BEHR', 'Workspaces','SE US BEHR Hourly - No ghost'),@(x) exist(x,'dir'))
...
for d=1:numel(dnums);
    ...
    if init_bool
        ...
        blank_mat = nan(sum(yy), sum(xx), numel(dnums)*4);
    ...
    Sectors.(todays_sect).sum_weighted_col = Sectors.(todays_sect).sum_weighted_col + this_WeightedColumn;
    Sectors.(todays_sect).sum_weight = Sectors.(todays_sect).sum_weight + this_Weight;
end
