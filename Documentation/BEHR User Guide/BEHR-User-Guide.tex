...
Starting from v3.0A, we have added our own flags field that combine the NASA XTrackQualityFlags and VcdQualityFlags summary bits with processing errors or warnings from the BEHR algorithm. The meaning of each bit is given 
...
1 & 1 & Summary bit: set if any error present \\
2 & 2 & AMF error: set if either the BEHR AMF or BEHR visible-only AMF is less than or equal to the minimum allowed value \\
3 & 4 & VcdQualityFlags: set if the VcdQualityFlags field was not an even integer (i.e. its own summary bit was set) \\
4 & 8 & XTrackFlags: set if the XTrackFlags field was $> 0$, indicating the presence of the row anomaly \\ 
5--16 & 16--32768 & Unused: reserved for future use \\ \hline
\multicolumn{3}{c}{\textbf{Warning bits}} \\
Bit position & Bit value & \multicolumn{1}{c}{Bit meaning} \\ \hline
17 & 65536 & MODIS ocean flag: MODIS albedo data not present for this pixel, used reflectance from \citet{mobley15}, scaled by the ratio of reflectance at 433 nm to 550 nm based on the GOME-2A LER product (\url{http://www.temis.nl/surface/gome2_ler.html}). \\ 
18--32 & $2^{17}$--$2^{31}$ & Unused: reserved for future use \\ \hline
...
\section{Considerations when working with BEHR files}
	...
	For users wishing to identify high quality \ce{NO2} column information to ground, we have added our own flag field that combines relevant flags from NASA with processing flags from our algorithm. This is the BEHRQualityFlags field.  In general, you should not use pixels for which the BEHRQualityFlags field is not an even integer (the least significant bit is set). For more advanced users, the meaning of each bit is described in Table \ref{tab:behr-quality-flags}.  Additionally, pixels with fill values should clearly not be used.
	
	In addition, users should filter pixels by cloud fraction. BEHR contains three cloud fractions: OMI geometric, OMI radiance, and MODIS cloud fraction. We generally filter for OMI geometric or MODIS fraction to be $< 0.2$ (20\%) when interested in total (to ground) columns.  As discussed in Russell et al. 2011, MODIS cloud fraction is often less susceptible to identifying high albedo ground surfaces as clouds. Other applications (e.g. cloud slicing) may require different filtering.
	...
	Other fields not containing \ce{NO2} data are gridded by the constant value method (CVM), also available in the \lstinline$omi$ package (\url{https://github.com/gkuhl/omi}). This avoids assuming that the same constraints optimized for \ce{NO2} in the PSM are true for all fields. These fields are not automatically filtered for row anomaly or other processing errors. Consequently, before averaging in time (\S\ref{sec:temporal-averaging}), the user should filter this data similarly to the native pixels (\S\ref{sec:native-filtering}). These fields are indicated by the \lstinline$grid_type$ attribute having the value ``constant value method''.
	...
	Averaging over time is most easily accomplished using the gridded product, since the grid is consistent day-to-day. CVM gridded fields (\S\ref{sec:gridded:cvm}) should be filtered for quality using BEHRQualityFlags or XTrackQualityFlags and VcdQualityFlags and filtered for cloud fraction. The temporal average should weight each grid cell by the weight given in the Areaweight field for that swath.
	...
	The difference in the VCDs stems from the difference in the computation of the cloudy AMF. The total-column AMF (BEHRAMFTrop) is ultimately the ratio of a modeled slant column density (accounting for the presence of clouds) to a modeled vertical column density that is integrated from ground to tropopause over the whole pixel:
	
	\begin{equation}\label{eqn:total-amf}
	A_{\mathrm{total}} = \frac{(1-f_r) \int_{p_{\mathrm{surf}}}^{p_{\mathrm{tp}}} w_{\mathrm{clear}}(p) g(p) \: dp + f_r \int_{p_{\mathrm{cloud}}}^{p_{\mathrm{tp}}} w_{\mathrm{cloudy}}(p) g(p) \: dp}{\int_{p_{\mathrm{surf}}}^{\mathrm{p_{tp}}} g(p) \: dp}
	\end{equation}
