...
{(1-f_g)\int_{p_{\mathrm{surf}}}^{\mathrm{p_{tp}}} g(p) \: dp + f_g \int_{p_{\mathrm{cloud}}}^{\mathrm{p_{tp}}} g(p) \: dp}
	...
	\textbf{Please note:} the v3.0 averaging kernels are still in the process of being validated. Users should contact the BEHR maintainer listed on the website to verify their accuracy before using them in any study.
	...
	where $AK_i$ is the averaging kernel and $g_i$ the model \ce{NO2} partial column density for level $i$. The model profile should be interpolated to the pressure levels given in BEHRPressureLevels, rather than the other way around, since the averaging kernels can have sharp transitions at clouds. Since $AK_i = w_i/A$, where $w_i$ is the scattering weight for level $i$ and $A$ the pixel's AMF, this calculation effectively converts the model column to a slant column first, then ``retrieves'' it using the BEHR AMF. \textbf{Note:} since BEHR provides a tropospheric \ce{NO2} column density, it must be compared against a modeled tropospheric column only.
	...
	\subsubsection{Justification for including terrain and cloud pressure levels}
	
	Setting the clear and cloudy scattering weights to zero below the terrain and cloud top, respectively, represents the assumed zero sensitivity of the satellite to \ce{NO2} below the cloudy part of the pixel, and that the \ce{NO2} column necessarily does not extend below ground. This results in a sharp discontinuity in the averaging kernels at each of these pressures. By including these pressure levels in the averaging kernel vectors, we place that discontinuity at the exact pressure our algorithm does internally, rather than the next pressure level above.
