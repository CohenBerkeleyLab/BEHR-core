...
that is, the averaging kernel for pixel $i$ at pressure $p$ is the cloud radiance fraction weighted average of the clear and cloudy scattering weights for pixel $i$ at pressure $p$, divided by the to-ground AMF for pixel $i$.
	...
	Similarly, scattering weights can be used with Eqs. \eqref{eqn:total-amf} and \eqref{eqn:vis-amf} to calculate custom AMFs with new \emph{a priori} profiles. The published scattering weights include any necessary temperature correction, so they can be used directly in Eqs.\eqref{eqn:total-amf} and \eqref{eqn:vis-amf}.
	
	We strongly recommend that users interested in calculating their own AMFs check their code by attempting to reproduce the BEHR AMFs from the published scattering weights and \ce{NO2} profiles. Using \path{integPr2.m} from the \href{https://github.com/CohenBerkeleyLab/BEHR-core-utils}{BEHR core utilities repository}, one should be able to reproduce the BEHR AMFs nearly exactly. \path{test_published_scattering_weights.m} in the \href{https://github.com/CohenBerkeleyLab/BEHR-core}{BEHR core repository} demonstrates how BEHR AMFs can be reproduced using the published scattering weights and \ce{NO2} profiles. 
	
	Two final notes: first, \path{integPr2.m} integrates the \emph{a priori} profile in mixing ratio (or mixing ratio times scattering weight) over pressure, see Appendix B of \citet{ziemka01} for information on how mixing ratio integrated over pressure should be done to be equivalent to number density integrated over altitude. Second, the clear and cloudy scattering weights are set to 0 below the surface and cloud pressures, respectively before they are combined to yield the averaging kernel, so there will implicitly be no below-ground or below-cloud contribution to the respective subscenes.
